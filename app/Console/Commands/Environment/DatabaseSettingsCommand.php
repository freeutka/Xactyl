...
namespace Xactyl\Console\Commands\Environment;

use Illuminate\Console\Command;
use Illuminate\Contracts\Console\Kernel;
use Illuminate\Database\DatabaseManager;
use Xactyl\Traits\Commands\EnvironmentWriterTrait;
...
class DatabaseSettingsCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->variables['DB_USERNAME'] = $this->option('username') ?? $this->ask(
            'Database Username',
            config('database.connections.mysql.username', 'xactyl')
        );
        ...
        try {
            ...
            if ($this->confirm('Go back and try again?')) {
                $this->database->disconnect('_xactyl_command_test');
    ...
    {
        config()->set('database.connections._xactyl_command_test', [
            'driver' => 'mysql',
        ...
        $this->database->connection('_xactyl_command_test')->getPdo();
    }
}
